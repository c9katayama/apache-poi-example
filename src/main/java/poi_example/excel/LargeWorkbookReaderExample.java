...
/**
 * 大量のデータを含むExcelファイルを効率的に読み込むサンプル。 イベントモデルを使用して、メモリ使用量を抑えながら処理を行います。
 */
public class LargeWorkbookReaderExample {
	...
	public static void main(String[] args) throws Exception {
		...
		try (InputStream is = new FileInputStream(file); OPCPackage pkg = OPCPackage.open(is)) {
			...
			ContentHandler handler = new XSSFSheetXMLHandler(styles, sst, new ExampleContentsHandler(),
					new DataFormatter(), false // フォーミュラの評価を行わない
			);
